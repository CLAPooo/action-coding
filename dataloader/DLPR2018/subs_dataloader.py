...
class subs_dataset(Dataset):  
    ...
    def stackopf(self):
        name = 'v_'+self.video
	subfolder = self.video.split('_',1)[0]
        tmp = self.video.split('_',1)[1]
        if subfolder == 'HandstandPushups':
            subfolder = 'HandStandPushups'
            subfolder = subfolder+'_'+tmp
	    name = 'v_'+subfolder

        u = self.root_dir+'/'+self.video.split('_',1)[0]+'/' +name
	#v = self.root_dir+self.video.split('_',1)[0]+'/'+ name
	#print u
...
class subs_DataLoader():
    ...
    def load_frame_count(self):
        ...
        with open('/home/chao/chao/action/pytorch/two-stream-action-recognition/dataloader/dic/frame_count.pickle','rb') as file:
    ...
    def train(self):
        training_set = subs_dataset(dic=self.dic_video_train, in_channel=self.in_channel, root_dir=self.data_path,
            mode='train',
    ...
    def val(self):
        validation_set = subs_dataset(dic= self.dic_test_idx, in_channel=self.in_channel, root_dir=self.data_path ,
            mode ='val',
            transform = transforms.Compose([
            transforms.Resize([224,224]),
            transforms.ToTensor(),
...
#                                        path='/home/chao/chao/action/dataset/my_ucf_grey/',
